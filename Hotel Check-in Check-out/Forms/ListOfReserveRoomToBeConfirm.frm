...
Public Sub AddFunction(lstView As ListView, idnum As String)
Dim a As Integer, lstItem As ListItem
If rs.State = adStateOpen Then rs.Close
msql = "Select * from tblRoom where id='" & idnum & "'"
rs.Open msql, conn
Do While Not rs.EOF
    a = a + 1
    Set lstItem = lstView.ListItems.Add(, , a)
        lstItem.SubItems(0) = rs(0).Value
